                ...
                <div class="row">
                    ...
                    <!-- &fieldSearch=users.id&contentSearch=1 -->
                        <?php
                            ...
                            $allEnableClass = (!isset($_GET['enable']) || $_GET['enable'] == 'all') ? 'primary' : 'danger';
                            $activeEnableClass = (isset($_GET['enable']) && $_GET['enable'] == 'active') ? 'primary' : 'danger';
                            $inactiveEnableClass = (isset($_GET['enable']) && $_GET['enable'] == 'inactive') ? 'primary' : 'danger';



                        ?>
                        <a href="?controller=<?php echo $this->controllerName ?>&action=list&enable=all<?php echo $link ?>" class="btn btn-<?php echo  $allEnableClass ?> mr-2">
                            Tất cả
                            <span class="badge bg-white"><?php echo $countAll ?></span>
                        </a>
                        <a href="?controller=<?php echo $this->controllerName ?>&action=list&enable=active<?php echo $link ?>" class="btn btn-<?php echo  $activeEnableClass ?> mr-2">
                            Kích hoạt
                            <span class="badge bg-white"><?php echo $countActive ?></span>
                        </a>
                        <a href="?controller=<?php echo $this->controllerName ?>&action=list&enable=inactive<?php echo $link ?>" class="btn btn-<?php echo  $inactiveEnableClass ?>">
                            Chưa kích hoạt
